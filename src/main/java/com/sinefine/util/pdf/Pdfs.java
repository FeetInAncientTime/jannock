 ...
 * 
 * <p>
 * Often, a simple byte by byte comparison cannot be performed on a generated 
 * PDF file and the expected PDF file as both files contain information that is
 * specific to the exact environment in which they were created and the time at
 * which they were created. Hence, this class provides more lenient methods of
 * comparison.
 * </p>
 *
 * <p>
 ...
 *
 * TODO Consider adding a method which returns a diff list.
...
public final class Pdfs {
    ...
    /**
     * The {@code Configuration} class represents the configuration to be used
     * by methods of the class.
     * 
     * <p>The configuration includes the lines (normal and array) to ignore in 
     * PDF document.
     * </p>
    ...
    static final class Configuration {
        ...
        public Configuration(final Set<String> arrayLinePrefixesToIgnore,
            ...
            this.arrayLinePrefixesToIgnore = Collections.unmodifiableSet(arrayLinePrefixesToIgnore);
            this.linePrefixesToIgnore = Collections.unmodifiableSet(linePrefixesToIgnore);
        }
    ...
    static final Configuration DEFAULT_CONFIGURATION =
            new Configuration(DEFAULT_ARRAY_LINE_PREFIXES_TO_IGNORE, DEFAULT_LINE_PREFIXES_TO_IGNORE);
    ...
    /**
     * Returns {@code true} if the two PDF InputStreams are equal, {@code false}
     * otherwise.
     *
     * <p>
     * This method is equivalent to:</p>
     * <ul>
     * <li>{@linkplain #areEqual(byte[], byte[])}, where the byte arrays
     * represent the input streams.</li>
     * </ul>
     *
     * @param actual The first PDF input stream
     * @param expected The second PDF input stream
     * @return {@code true} if the two PDF InputStreams are equal, {@code false}
     * otherwise.
     * @throws IOException if an error occurs whilst processing the input
     * streams.
     */
    ...
    /**
     * Returns {@code true} if the two PDF byte arrays are equal, {@code false}
     * otherwise.
     *
     * <p>
     * Two PDF documents are considered equal if either of the following methods
     * returns {@code true}.</p>
     * <ul>
     ...
     * Pdfs#areContentsEqual(byte[], byte[])} <em>is always evaluated 
     * before</em> the method {@link #areImagesSame(InputStream, InputStream) }.
     * </p>
     *
     * @param actual The first PDF byte array
     * @param expected The second PDF byte array
     * @return {@code true} if the two PDF byte arrays are equal, {@code false}
     * otherwise.
    ...
    /**
     * Returns {@code true} if the contents of the two PDF InputStreams are
     * equal, {@code false} otherwise.
     *
     * <p>
     * A simple byte by byte comparison cannot be performed on the generated PDF
     * file and the expected PDF file as both files contain information that is
     * specific to the exact environment in which they were created and the time
     * at which they were created.
     * </p>
     *
     * <p>
     * This method attempts to avoid these problems by ignoring lines that start
     * with the following environment specific tags:
     * </p>
     * <ul>
     * <li>/Producer</li>
     * <li>/Creator</li>
     ...
     *
     * <p>
     * If an item in the above list appears as a dictionary object at the start
     * of a line then it is also ignored. Dictionary objects are identified by
     * the use of the {@literal <<} symbols.
     * </p>
     *
     * <p>
     * Commented lines (i.e. lines that begin with the text '%') are also
     * ignored. If the comment is <em>not</em> the very first character of the
     * line then the line <em>will</em> be included in the test.
     * </p>
     *
     * <p>
     ...
     * </ul>
     * <p>
     * In the case of the above tags, subsequent lines will also be ignored
     * until a line contains the symbol ']'. After that subsequent lines will be
     * processed as before.
     * </p>
     * <p>
     ...
     * @param actual The first PDF input stream
     * @param expected The second PDF input stream
     * @return {@code true} if the contents of two input streams are equal,
     * {@code false} otherwise.
     * @throws java.io.IOException if an error occurs whilst reading the input
     * streams.
     */
    ...
    /**
     * Returns {@code true} if the contents of the two PDF InputStreams are
     * equal, {@code false} {@literal otherwise.}  Unlike 
     * {@link #areContentsEqual}, this method allows clients to specify the
     * configuration.
     *
     * @param actual The first PDF input stream
     * @param expected The second PDF input stream
     * @param configuration the configuration to use.
     * @return {@code true} if the contents of the two PDF InputStreams are
     * equal, {@code false} {@literal otherwise.}
     * @throws IOException if an error occurs whilst reading the input
     * streams.
     */
    static boolean areContentsEqual(final InputStream actual,
            final InputStream expected, final Configuration configuration) throws IOException {
                        ...
                        if (!line1.equals(line2)) {
                            if (isInIgnoredArray) {
                                ...
                                if (skipLine(line1, line2, arrayLinePrefixesToIgnore)) {
                                    ...
                                    logger.error("The following lines [#" + lineNumber
                                            + "] are different!\r\n\t"
                                            + "1. "
                                            + line1
                                            + "\r\n\t"
    ...
    /**
     * Returns {@code true} if the contents of the two PDF byte arrays are
     * equal, {@code false} otherwise.
     *
     * <p>
     * This method is equivalent to: </p>
     ...
     * @param actual The first PDF byte array
     * @param expected The second PDF byte array
     * @return {@code true} if the contents of the two PDF byte arrays are
     * equal, {@code false} otherwise.
     * @throws IOException if an error occurs whilst processing the byte arrays.
     * @see #areEqual(InputStream, InputStream)
     */
    ...
    /**
     * Returns {@code true} if the difference in size between the actual byte
     * array and the expected byte array is less than the tolerated difference.
     *
     * <p>
     * Visually identical PDF files may not have exactly the same size as the
     * generated PDF file and the expected PDF file may contain information that 
     * is specific to the exact environment in which they were created and the 
     * time at which they were created.
     * </p>
     *
     * <p>
     * The tolerance is expressed as a value between 0 and 1 inclusive. A zero
     * tolerance value indicates that the arrays must be the same size. The
     * maximum tolerance value of 1 indicates that the size of the actual byte
     * array can be any value between 1 and twice the size of the expected byte
     * array.</p>
     *
     * <p>
     ...
     * @return {@code true} if the difference in size between the actual byte
     * array and the expected byte array is less than the tolerated difference.
     * @throws IllegalArgumentException if the tolerance is less than zero or
     * greater than one.
     */
    ...
    /**
     * Returns {@code true} if the difference in size between the actual input
     * stream and the expected input stream is less than the tolerated
     * difference.
     *
     * <p>
     * This method is equivalent to the method
     * {@link #areContentsSimilarSize(byte[], byte[], float)} where the input
     * streams are converted to byte arrays.
     * </p>
     *
     * @param actual the actual input stream.
     * @param expected the expected input stream.
     * @param tolerance the tolerance.
     * @return {@code true} if the difference in size between the actual input
     * stream and the expected input stream is less than the tolerated
     * difference.
     * @throws IOException if an I/O error occurs whilst trying to read the 
     *   input streams.
     * @see #areContentsSimilarSize(byte[], byte[], float)
     */
    public static boolean areContentsSimilarSize(final InputStream actual,
            final InputStream expected, final float tolerance) throws IOException {
        if (tolerance < 0 || tolerance > 1) {
            throw new IllegalArgumentException(
                    "The tolerance (" + tolerance + ") must be between 0 and 1!");
        }
        if (actual == null || expected == null) {
            return false;
        } else {
    ...
    /**
     * Returns {@code true} if the <em>images</em> of the two PDF input streams
     * are the same, {@code false} otherwise.
     *
     * <p>
     * This method is equivalent to {@link #areImagesSame(byte[], byte[])} 
    ...
    /**
     * Returns {@code true} if the <em>images</em> of the two PDF byte arrays
     * are the same, {@code false} otherwise.
     *
     * <p>
     * Any differences between the documents that do not change the rendered
     * image are ignored.</p>
     *
     * @param actual the actual byte array.
     * @param expected the expected byte array.
     * @return {@code true} if the two PDF <em>images</em> are the same,
     * {@code false} otherwise.
     * @throws IOException if an error occurs whilst processing the byte arrays.
     */
    ...
    /**
     * Returns {@code true}, if the list of byte arrays are equal, {@code false}
     * otherwise.
     *
     * <p>
     ...
     * @return {@code true}, if the list of byte arrays are equal, {@code false}
     * otherwise.
     */
    private static boolean areByteListsEqual(List<byte[]> listX, List<byte[]> listY) {
    ...
    /**
     * Returns a list of byte arrays representing the pages of the PDF document
     * as images.
     * 
     * @param pdfDocument the PDF document.
     * @return a list of byte arrays representing the pages of the PDF document
     *   as images.
     * @throws IOException if an I/O error occurs during the reading of the 
     *   document or the creation of the page images.
     */
    ...
    private static boolean skipLine(final String line1, final String line2,
            final Set<String> linePrefixesToIgnore) {
        return linePrefixesToIgnore.stream().anyMatch(
